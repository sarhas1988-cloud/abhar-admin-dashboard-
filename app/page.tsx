...
const palette = ['#d8573a', '#2a211c', '#e8b8a5', '#d4c4b0', '#b8752f']
...
export default function HomePage() {
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#faf6f0] text-[#2a211c]">
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-[252px] flex-col border-l border-[#e8dfd3] bg-white px-5 py-6 transition-transform duration-300 lg:translate-x-0 ${menuOpen ? 'translate-x-0' : 'translate-x-[110%]'}`}>
        <div className="flex items-center justify-between pb-8">
          ...
          <button onClick={() => setMenuOpen(false)} className="rounded-lg p-2 text-[#8a7969] hover:bg-[#f5ede2] lg:hidden" aria-label="إغلاق القائمة"><X /></button>
        </div>
        <p className="mb-3 px-3 text-[11px] font-semibold tracking-[0.16em] text-[#a3907e]">القائمة الرئيسية</p>
        <nav className="flex flex-col gap-1.5">
          {navItems.map(({ label, href, icon: Icon }) => <Link key={href} href={href} onClick={() => setMenuOpen(false)} className={`flex items-center gap-3 rounded-xl px-3.5 py-3 text-right text-sm font-medium transition ${pathname === href ? 'bg-[#faf1eb] text-[#d8573a]' : 'text-[#6b5d53] hover:bg-[#faf6f0] hover:text-[#2a211c]'}`}><Icon size={19} strokeWidth={1.8} /><span>{label}</span>{href === '/orders' && pendingOrders > 0 && <span className="mr-auto rounded-full bg-[#d8573a] px-2 py-0.5 text-[10px] font-bold text-white">{pendingOrders}</span>}</Link>)}
        </nav>
        <div className="mt-auto flex flex-col gap-1.5 border-t border-[#ede4d7] pt-4">
          <button onClick={signOut} className="flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium text-[#6b5d53] hover:bg-[#faf6f0] hover:text-[#c04a2f]"><LogOut size={19} strokeWidth={1.8} />تسجيل الخروج</button>
          <div className="mt-2 flex items-center gap-3 rounded-xl bg-[#faf6f0] p-3"><div className="flex size-9 items-center justify-center rounded-full bg-[#2a211c] text-xs font-bold text-white">{isAdmin ? 'أ' : 'م'}</div><div className="min-w-0"><p className="truncate text-xs font-semibold">{isAdmin ? 'الأدمن' : 'موظف'}</p><p className="truncate text-[10px] text-[#8a7969]">{email}</p></div></div>
        </div>
      </aside>
      ...
      <section className="lg:mr-[252px]">
        <header className="flex h-[84px] items-center justify-between border-b border-[#e8dfd3] bg-white px-5 sm:px-8">
          <div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-lg bg-[#faf1eb] p-1 lg:hidden"><img src="/abhar-logo.svg" alt="إبهار" className="h-full w-full object-contain" /></div><button onClick={() => setMenuOpen(true)} className="rounded-lg p-2 text-[#6b5d53] hover:bg-[#f5ede2] lg:hidden" aria-label="فتح القائمة"><Menu /></button><div><h1 className="font-serif mt-1 text-xl font-semibold tracking-tight sm:text-2xl">مرحباً، {isAdmin ? 'الأدمن' : email}</h1></div></div>
          <Bell size={19} strokeWidth={1.8} className="text-[#6b5d53]" />
        </header>
        <div className="mx-auto max-w-[1400px] p-5 pb-24 sm:p-8 lg:pb-8">
          <div className="mb-10 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div><h2 className="font-serif text-3xl font-semibold sm:text-4xl">لوحة التحكم</h2><p className="mt-2 text-sm text-[#8a7969]">تابع حركة النشر والطباعة والمخزن من مكان واحد.</p></div>
          ...
          {categoryBreakdown.length > 0 && (
            <div className="mt-8 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
              <section className="rounded-2xl border border-[#e8dfd3] bg-white p-6 sm:p-8 xl:col-span-2">
                <div className="mb-6 flex items-center justify-between"><div><h3 className="font-bold">توزيع التصنيفات</h3><p className="mt-1 text-xs text-[#a3907e]">الكتب حسب النوع</p></div></div>
                <div className="flex flex-col items-center gap-7 sm:flex-row sm:justify-center">
                  <div className="relative flex size-[142px] items-center justify-center rounded-full" style={{ background: `conic-gradient(${gradientStops})` }}>
                    <div className="flex size-[94px] items-center justify-center rounded-full bg-white text-center"><div><strong className="block text-2xl">{books.length}</strong><span className="text-[10px] text-[#a3907e]">كتاب</span></div></div>
                  </div>
                  <div className="flex flex-col gap-3 text-xs text-[#6b5d53]">{categoryBreakdown.map(c => <Legend key={c.label} color={c.color} text={c.label} value={`${c.pct}%`} />)}</div>
                </div>
              </section>
            </div>
          )}

          <section className="mt-8 rounded-2xl border border-[#e8dfd3] bg-white p-6 sm:p-8">
            <div className="mb-5"><h3 className="font-bold">أحدث الكتب</h3><p className="mt-1 text-xs text-[#a3907e]">آخر الكتب المضافة إلى مكتبتك</p></div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[650px] text-right text-sm">
                <thead><tr className="border-b border-[#ede4d7] text-xs text-[#a3907e]"><th className="pb-3 font-medium">الكتاب</th><th className="pb-3 font-medium">المؤلف</th><th className="pb-3 font-medium">التصنيف</th><th className="pb-3 font-medium">عدد النسخ</th><th className="pb-3 font-medium">تاريخ التعاقد</th></tr></thead>
                <tbody>{books.map(book => (
                  <tr key={book.id} className="border-b border-[#f0e7db] last:border-0">
                    <td className="py-4"><div className="flex items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#faf1eb]"><BookOpen size={17} className="text-[#d8573a]" /></div><p className="font-semibold">{book.title}</p></div></td>
                    <td className="py-4 text-[#6b5d53]">{book.book_authors?.map(a => a.authors.name).join('، ')}</td>
                    <td className="py-4 text-[#6b5d53]">{book.category}</td>
                    <td className="py-4 font-medium">{book.printed_copies?.toLocaleString('ar-EG')}</td>
                    <td className="py-4 text-xs text-[#6b5d53]">{book.contract_date}</td>
                  </tr>
                ))}</tbody>
              </table>
              {!loading && books.length === 0 && <p className="py-10 text-center text-sm text-[#a3907e]">لا توجد كتب مسجلة بعد — ابدئي بإضافة كتاب من التعاقدات.</p>}
            </div>
          </section>
        </div>
      </section>
...
function Stat({ title, value, icon: Icon }: { title: string; value: string; icon: typeof BookOpen }) {
  return <div className="rounded-2xl border border-[#e8dfd3] bg-white p-5"><div className="flex items-start justify-between"><div><p className="text-xs text-[#8a7969]">{title}</p><p className="mt-3 text-xl font-bold tracking-tight sm:text-2xl">{value}</p></div><div className="flex size-10 items-center justify-center rounded-xl bg-[#faf1eb] text-[#d8573a]"><Icon size={19} /></div></div></div>
}
function Legend({ color, text, value }: { color: string; text: string; value: string }) {
  return <div className="flex items-center gap-2"><span className="size-2 rounded-full" style={{ background: color }} /><span>{text}</span><strong className="mr-auto text-[#2a211c]">{value}</strong></div>
}
