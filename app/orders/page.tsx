  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-[#1a2540]">
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-[252px] flex-col border-l border-[#e8ebf0] bg-white px-5 py-6 transition-transform lg:translate-x-0 ${menuOpen ? 'translate-x-0' : 'translate-x-[110%]'}`}>
        ...
        <nav className="flex flex-col gap-1.5">{navItems.map(({ label, href, icon: Icon }) => <Link key={href} href={href} onClick={() => setMenuOpen(false)} className={`flex items-center gap-3 rounded-xl px-3.5 py-3 text-sm font-medium ${pathname === href ? 'bg-[#fff0ed] text-[#d8573a]' : 'text-[#69758a] hover:bg-[#f7f8fa]'}`}><Icon size={19} /><span>{label}</span>{href === '/orders' && pending.length > 0 && <span className="mr-auto rounded-full bg-[#d8573a] px-2 py-0.5 text-[10px] font-bold text-white">{pending.length}</span>}</Link>)}</nav>
      ...
      <section className="lg:mr-[252px]">
        <header className="flex h-[84px] items-center justify-between border-b border-[#e8ebf0] bg-white px-5 sm:px-8"><div className="flex items-center gap-3"><button onClick={() => setMenuOpen(true)} className="lg:hidden" aria-label="فتح القائمة"><Menu /></button><div><p className="text-xs text-[#8d97a7]">طلبات العملاء</p><h1 className="mt-1 text-xl font-bold sm:text-2xl">الاوردرات</h1></div></div><Bell size={19} className="text-[#69758a]" /></header>
        <div className="mx-auto max-w-[1400px] p-5 pb-24 sm:p-8 lg:pb-8">
          {!accessLoading && !canView('orders') ? <p className="rounded-xl border border-[#e8ebf0] bg-white p-6 text-center text-sm text-[#9ba4b2]">مفيش صلاحية وصول لهذا القسم.</p> : (
            <>
              <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-[#d8573a]"><span className="size-2 rounded-full bg-[#d8573a]" />متابعة كل الطلبات</div><h2 className="text-2xl font-bold sm:text-3xl">الأوردرات</h2><p className="mt-2 text-sm text-[#8d97a7]">سجل كل طلب عميل وحالة تسليمه.</p></div>{canEdit('orders') && <button onClick={openCreate} className="flex w-fit items-center gap-2 rounded-xl bg-[#d8573a] px-4 py-3 text-sm font-semibold text-white"><Plus size={18} />إضافة أوردر جديد</button>}</div>
              <div className="mb-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-[#e8ebf0] bg-white p-5"><p className="text-xs text-[#8d97a7]">إجمالي الأوردرات</p><p className="mt-2 text-2xl font-bold">{orders.length}</p></div><div className="rounded-2xl border border-[#e8ebf0] bg-white p-5"><p className="text-xs text-[#8d97a7]">قيد التسليم</p><p className="mt-2 text-2xl font-bold text-[#b26b16]">{pending.length}</p></div><div className="rounded-2xl border border-[#e8ebf0] bg-white p-5"><p className="text-xs text-[#8d97a7]">متأخرة (أكثر من ٧ أيام)</p><p className="mt-2 text-2xl font-bold text-[#c84c3b]">{overdue.length}</p></div></div>
              <section className="rounded-2xl border border-[#e8ebf0] bg-white">
                <div className="flex flex-col gap-3 border-b border-[#eef0f3] p-4 sm:flex-row sm:items-center"><label className="flex min-w-[220px] flex-1 items-center gap-2 rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs text-[#9ba4b2]"><Search size={15} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="ابحث باسم العميل أو الكتاب" className="w-full bg-transparent outline-none" /></label><select value={deliveredFilter} onChange={e => setDeliveredFilter(e.target.value)} className="rounded-lg border border-[#e8ebf0] bg-white px-3 py-2 text-xs"><option value="all">كل الحالات</option><option value="delivered">تم التسليم</option><option value="pending">قيد التسليم</option></select><select value={sourceFilter} onChange={e => setSourceFilter(e.target.value)} className="rounded-lg border border-[#e8ebf0] bg-white px-3 py-2 text-xs"><option value="all">كل المصادر</option>{sources.map(s => <option key={s}>{s}</option>)}</select><span className="text-xs text-[#9ba4b2]">{loading ? 'جارٍ التحميل...' : `${filtered.length} أوردر`}</span></div>
                <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-right text-sm"><thead><tr className="border-b border-[#eef0f3] text-xs text-[#9ba4b2]"><th className="px-5 py-4">اسم الكتاب</th><th className="px-5 py-4">اسم العميل</th><th className="px-5 py-4">رقم التليفون</th><th className="px-5 py-4">السعر</th><th className="px-5 py-4">مصدر الأوردر</th><th className="px-5 py-4">تاريخ الاستلام</th><th className="px-5 py-4">تم التسليم</th><th className="px-5 py-4">إجراء</th></tr></thead><tbody>{filtered.map(order => <tr key={order.id} className="border-b border-[#f1f3f5] last:border-0"><td className="px-5 py-4 font-semibold">{order.books?.title}</td><td className="px-5 py-4 text-[#69758a]">{order.customer_name}</td><td className="px-5 py-4 text-[#69758a]">{order.customer_phone}</td><td className="px-5 py-4 text-[#69758a]">{order.price?.toLocaleString('ar-EG')} ج.م</td><td className="px-5 py-4 text-[#69758a]">{order.source}</td><td className="px-5 py-4 text-xs text-[#69758a]">{order.order_date}</td><td className="px-5 py-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${order.delivered ? 'bg-[#e9f7ef] text-[#25824a]' : 'bg-[#fff4e5] text-[#b26b16]'}`}>{order.delivered ? 'نعم' : 'لا'}</span></td><td className="px-5 py-4">{canEdit('orders') && <button onClick={() => openEdit(order)} className="rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs font-semibold text-[#d8573a]">تعديل</button>}</td></tr>)}</tbody></table>{!loading && filtered.length === 0 && <p className="p-8 text-center text-sm text-[#9ba4b2]">لا توجد أوردرات مسجلة بعد.</p>}</div>
              </section>
            </>
          )}
        </div>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1a2540]/30 p-3 sm:p-6">
          <section className="my-3 w-full max-w-2xl rounded-2xl bg-white shadow-2xl sm:my-8">
            <div className="flex items-center justify-between border-b border-[#eef0f3] p-5"><div><h2 className="text-xl font-bold">{editing ? 'تعديل الأوردر' : 'إضافة أوردر جديد'}</h2></div><button onClick={() => setFormOpen(false)} aria-label="إغلاق"><X /></button></div>
