...
const statusStyle: Record<Status, string> = { 'متاح': 'bg-[#e9f7ef] text-[#25824a]', 'غير متاح': 'bg-[#fce8e6] text-[#c84c3b]', 'قيد المراجعة': 'bg-[#fff4e5] text-[#b26b16]' }
  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-[#1a2540]">
      ...
      <section className="lg:mr-[252px]">
        <header className="flex h-[84px] items-center justify-between border-b border-[#e8ebf0] bg-white px-5 sm:px-8"><div className="flex items-center gap-3"><button onClick={() => setMenuOpen(true)} className="lg:hidden" aria-label="فتح القائمة"><Menu /></button><div><p className="text-xs text-[#8d97a7]">متابعة الكتب على منصات البيع</p><h1 className="mt-1 text-xl font-bold sm:text-2xl">المنصات</h1></div></div><Bell size={19} className="text-[#69758a]" /></header>
        <div className="mx-auto max-w-[1400px] p-5 pb-24 sm:p-8 lg:pb-8">
          {!accessLoading && !canView('platforms') ? <p className="rounded-xl border border-[#e8ebf0] bg-white p-6 text-center text-sm text-[#9ba4b2]">مفيش صلاحية وصول لهذا القسم.</p> : (
            <>
              ...
              {canEdit('platforms') && (
                <section className="mb-6 rounded-2xl border border-[#e8ebf0] bg-white p-5 sm:p-6">
                  ...
                  <div className="flex flex-wrap gap-2">{platforms.map(p => <span key={p.id} className="rounded-full bg-[#f7f8fa] px-3 py-1.5 text-xs font-medium text-[#69758a]">{p.name}</span>)}</div>
                  <div className="mt-4 flex gap-2"><input value={newPlatform} onChange={e => setNewPlatform(e.target.value)} placeholder="اسم منصة جديدة" className="min-w-0 flex-1 rounded-lg border border-[#e8ebf0] px-3 py-2 text-sm outline-none focus:border-[#d8573a] sm:max-w-xs" /><button onClick={addPlatform} className="flex items-center gap-1.5 rounded-lg bg-[#fff0ed] px-3 py-2 text-xs font-semibold text-[#d8573a]"><Plus size={15} />إضافة منصة</button></div>
                </section>
              )}
              <section className="rounded-2xl border border-[#e8ebf0] bg-white">
                <div className="flex flex-col gap-3 border-b border-[#eef0f3] p-4 sm:flex-row sm:items-center"><label className="flex min-w-[220px] flex-1 items-center gap-2 rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs text-[#9ba4b2]"><Search size={15} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="ابحث باسم الكتاب" className="w-full bg-transparent outline-none" /></label><span className="text-xs text-[#9ba4b2]">{loading ? 'جارٍ التحميل...' : `${filteredBooks.length} كتب`}</span></div>
                <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-right text-sm"><thead><tr className="border-b border-[#eef0f3] text-xs text-[#9ba4b2]"><th className="px-5 py-4">اسم الكتاب</th>{platforms.map(p => <th key={p.id} className="px-3 py-4">{p.name}</th>)}<th className="px-5 py-4">إجراء</th></tr></thead><tbody>{filteredBooks.map(book => <tr key={book.id} className="border-b border-[#f1f3f5] last:border-0"><td className="px-5 py-4"><p className="font-semibold">{book.title}</p><p className="text-xs text-[#9ba4b2]">{book.book_authors?.map(a => a.authors.name).join('، ')}</p></td>{platforms.map(p => { const s = statusFor(book.id, p.id)?.status ?? 'غير متاح'; return <td key={p.id} className="px-3 py-4"><span className={`rounded-full px-2.5 py-1 text-[11px] font-medium ${statusStyle[s]}`}>{s}</span></td> })}<td className="px-5 py-4">{canEdit('platforms') && <button onClick={() => setEditingBook(book)} className="rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs font-semibold text-[#d8573a]">تحديث الحالة</button>}</td></tr>)}</tbody></table>{!loading && filteredBooks.length === 0 && <p className="p-8 text-center text-sm text-[#9ba4b2]">لا توجد كتب مسجلة بعد.</p>}</div>
              </section>
            </>
          )}
        </div>
      ...
      {editingBook && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1a2540]/30 p-3 sm:p-6">
          <section className="my-3 w-full max-w-2xl rounded-2xl bg-white shadow-2xl sm:my-8">
            <div className="flex items-center justify-between border-b border-[#eef0f3] p-5"><div><h2 className="text-xl font-bold">{editingBook.title}</h2><p className="mt-1 text-xs text-[#9ba4b2]">تحديث الحالة على كل منصة</p></div><button onClick={() => setEditingBook(null)} aria-label="إغلاق"><X /></button></div>
            <div className="flex flex-col gap-4 p-5">
              {platforms.map(p => {
                const current = statusFor(editingBook.id, p.id)
                return (
                  <div key={p.id} className="rounded-xl border border-[#e8ebf0] p-4">
                    <div className="mb-3 flex items-center justify-between"><span className="text-sm font-semibold">{p.name}</span>
                      <select defaultValue={current?.status ?? 'غير متاح'} onChange={e => updateStatus(editingBook.id, p.id, e.target.value as Status, current?.notes ?? '')} className="rounded-lg border border-[#e8ebf0] bg-white px-3 py-1.5 text-xs"><option>متاح</option><option>غير متاح</option><option>قيد المراجعة</option></select>
                    </div>
                    <input defaultValue={current?.notes ?? ''} onBlur={e => updateStatus(editingBook.id, p.id, current?.status ?? 'غير متاح', e.target.value)} placeholder="ملاحظات (اختياري)" className="w-full rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs outline-none focus:border-[#d8573a]" />
                  </div>
                )
              })}
              <div className="flex justify-end border-t border-[#eef0f3] pt-4"><button onClick={() => { setEditingBook(null); load() }} className="rounded-xl bg-[#d8573a] px-5 py-3 text-sm font-semibold text-white">تم</button></div>
            </div>
          </section>
        </div>
      )}
