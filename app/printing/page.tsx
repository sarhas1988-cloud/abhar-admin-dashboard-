  ...
  return (
    <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-[#1a2540]">
      ...
      <section className="lg:mr-[252px]">
        <header className="flex h-[84px] items-center justify-between border-b border-[#e8ebf0] bg-white px-5 sm:px-8"><div className="flex items-center gap-3"><button onClick={() => setMenuOpen(true)} className="lg:hidden" aria-label="فتح القائمة"><Menu /></button><div><p className="text-xs text-[#8d97a7]">إدارة عمليات الطباعة</p><h1 className="mt-1 text-xl font-bold sm:text-2xl">المطبعة</h1></div></div><Bell size={19} className="text-[#69758a]" /></header>
        <div className="mx-auto max-w-[1400px] p-5 pb-24 sm:p-8 lg:pb-8">
          {!accessLoading && !canView('printing') ? <p className="rounded-xl border border-[#e8ebf0] bg-white p-6 text-center text-sm text-[#9ba4b2]">مفيش صلاحية وصول لهذا القسم.</p> : (
            <>
              <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 flex items-center gap-2 text-xs font-medium text-[#d8573a]"><span className="size-2 rounded-full bg-[#d8573a]" />متابعة الكتب المسجلة</div><h2 className="text-2xl font-bold sm:text-3xl">كتب قيد الطباعة</h2><p className="mt-2 text-sm text-[#8d97a7]">تابع دخول واستلام الكتب من المطبعة وتسليم النسخ للكاتب.</p></div>{canEdit('printing') && <button onClick={openCreate} className="flex w-fit items-center gap-2 rounded-xl bg-[#d8573a] px-4 py-3 text-sm font-semibold text-white"><Plus size={18} />إضافة إلى المطبعة</button>}</div>
              <section className="rounded-2xl border border-[#e8ebf0] bg-white">
                ...
                <div className="overflow-x-auto"><table className="w-full min-w-[850px] text-right text-sm"><thead><tr className="border-b border-[#eef0f3] text-xs text-[#9ba4b2]"><th className="px-5 py-4">اسم الكتاب</th><th className="px-5 py-4">اسم الكاتب</th><th className="px-5 py-4">عدد النسخ</th><th className="px-5 py-4">دخول المطبعة</th><th className="px-5 py-4">استلام النسخ</th><th className="px-5 py-4">تم التسليم للكاتب</th><th className="px-5 py-4">إجراء</th></tr></thead><tbody>{filtered.map(job => <tr key={job.id} className="border-b border-[#f1f3f5] last:border-0"><td className="px-5 py-4 font-semibold">{job.books?.title}</td><td className="px-5 py-4 text-[#69758a]">{job.books?.book_authors?.map(a => a.authors.name).join('، ')}</td><td className="px-5 py-4 text-[#69758a]">{job.copies?.toLocaleString('ar-EG')}</td><td className="px-5 py-4 text-[#69758a]">{job.entered_at}</td><td className="px-5 py-4 text-[#69758a]">{job.received_at || 'لم تستلم بعد'}</td><td className="px-5 py-4"><span className={`rounded-full px-3 py-1 text-xs font-semibold ${job.delivered_to_author ? 'bg-[#e9f7ef] text-[#25824a]' : 'bg-[#fff4e5] text-[#b26b16]'}`}>{job.delivered_to_author ? 'نعم' : 'لا'}</span></td><td className="px-5 py-4">{canEdit('printing') && <button onClick={() => openEdit(job)} className="rounded-lg border border-[#e8ebf0] px-3 py-2 text-xs font-semibold text-[#d8573a]">تعديل</button>}</td></tr>)}</tbody></table>{!loading && filtered.length === 0 && <p className="p-8 text-center text-sm text-[#9ba4b2]">لا توجد كتب قيد الطباعة بعد.</p>}</div>
              </section>
            </>
          )}
        </div>
      ...
      {formOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1a2540]/30 p-3 sm:p-6">
          <section className="my-3 w-full max-w-2xl rounded-2xl bg-white shadow-2xl sm:my-8">
            <div className="flex items-center justify-between border-b border-[#eef0f3] p-5"><div><h2 className="text-xl font-bold">{editing ? 'تعديل بيانات الطباعة' : 'إضافة إلى المطبعة'}</h2><p className="mt-1 text-xs text-[#9ba4b2]">يتم اختيار الكتاب من الكتب المسجلة في التعاقدات فقط</p></div><button onClick={() => setFormOpen(false)} aria-label="إغلاق"><X /></button></div>
            <form onSubmit={submit} className="grid gap-4 p-5 sm:grid-cols-2">
              <label className="sm:col-span-2"><span className="mb-2 block text-xs font-semibold text-[#69758a]">اسم الكتاب</span><select required disabled={Boolean(editing)} value={form.bookId} onChange={e => update('bookId', e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] bg-white px-3 py-3 text-sm"><option value="">ابحث واختر كتاباً من التعاقدات</option>{books.map(book => <option key={book.id} value={book.id}>{book.title} — {book.book_authors?.map(a => a.authors.name).join('، ')}</option>)}</select></label>
              {selectedBook && <div className="sm:col-span-2 rounded-xl bg-[#fff8f6] p-3 text-xs text-[#69758a]">عدد النسخ في سجل الكتاب: <strong className="text-[#1a2540]">{selectedBook.printed_copies?.toLocaleString('ar-EG')}</strong> نسخة</div>}
              <label><span className="mb-2 block text-xs font-semibold text-[#69758a]">عدد النسخ</span><input required type="number" min="1" value={form.copies || selectedBook?.printed_copies || ''} onChange={e => update('copies', e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm outline-none focus:border-[#d8573a]" /></label>
              <label><span className="mb-2 block text-xs font-semibold text-[#69758a]">سعر النسخة من المطبعة</span><input required type="number" min="0" step="0.01" value={form.printerPrice} onChange={e => update('printerPrice', e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm outline-none focus:border-[#d8573a]" /></label>
              <label><span className="mb-2 block text-xs font-semibold text-[#69758a]">تاريخ دخول المطبعة</span><span className="relative block"><CalendarDays className="pointer-events-none absolute left-3 top-3 text-[#9ba4b2]" size={17} /><input required type="date" value={form.enteredAt} onChange={e => update('enteredAt', e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm" /></span></label>
              <label><span className="mb-2 block text-xs font-semibold text-[#69758a]">تاريخ استلام النسخ <em className="font-normal text-[#9ba4b2]">(اختياري)</em></span><input type="date" value={form.receivedAt} onChange={e => update('receivedAt', e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm" /></label>
              <label className="flex items-center justify-between rounded-xl border border-[#e8ebf0] p-4 sm:col-span-2"><span><span className="block text-sm font-semibold">تم تسليم الكاتب او لا</span></span><input type="checkbox" checked={form.delivered} onChange={e => update('delivered', e.target.checked)} className="size-5 accent-[#d8573a]" /></label>
