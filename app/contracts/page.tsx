    ...
    <main dir="rtl" className="min-h-screen bg-[#f7f8fa] text-[#1a2540]">
      <aside className={`fixed inset-y-0 right-0 z-40 flex w-[252px] flex-col border-l border-[#e8ebf0] bg-white px-5 py-6 transition-transform lg:translate-x-0 ${menuOpen ? 'translate-x-0' : 'translate-x-[110%]'}`}>
        ...
        <div className="mt-auto border-t border-[#edf0f3] pt-4">
          <button onClick={signOut} className="flex items-center gap-3 px-3.5 py-3 text-sm text-[#69758a] hover:text-[#c84c3b]"><LogOut size={19} />تسجيل الخروج</button>
        </div>
      </aside>
      ...
      <section className="lg:mr-[252px]">
        <header className="flex h-[84px] items-center justify-between border-b border-[#e8ebf0] bg-white px-5 sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-xs text-[#8d97a7]">القسم الفني والتعاقدات</p><h1 className="mt-1 text-xl font-bold sm:text-2xl">التعاقدات والقسم الفني</h1></div>
          </div>
          <Bell size={19} className="text-[#69758a]" />
        </header>

        <div className="mx-auto max-w-[1400px] p-5 pb-24 sm:p-8 lg:pb-8">
          {!accessLoading && !canView('contracts') ? (
            <p className="rounded-xl border border-[#e8ebf0] bg-white p-6 text-center text-sm text-[#9ba4b2]">مفيش صلاحية وصول لهذا القسم.</p>
          ) : (
            <>
              <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <div>
                  ...
                  <h2 className="text-2xl font-bold sm:text-3xl">كل الكتب</h2>
                  <p className="mt-2 text-sm text-[#8d97a7]">سجل الكتب المتعاقد عليها وتفاصيل القسم الفني.</p>
                </div>
              ...
              <section className="rounded-2xl border border-[#e8ebf0] bg-white">
                <div className="flex flex-col gap-3 border-b border-[#eef0f3] p-4 sm:flex-row sm:items-center">
                  ...
                  <select value={category} onChange={e => setCategory(e.target.value)} className="rounded-lg border border-[#e8ebf0] bg-white px-3 py-2 text-xs"><option>الكل</option>{categories.map(item => <option key={item}>{item}</option>)}</select>
                  <span className="text-xs text-[#9ba4b2]">{loading ? 'جارٍ التحميل...' : `${filteredBooks.length} كتب`}</span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[850px] text-right text-sm">
                    ...
                    <tbody>{filteredBooks.map(book => (
                      <tr key={book.id} onClick={() => setDetail(book)} className="cursor-pointer border-b border-[#f1f3f5] last:border-0 hover:bg-[#fcfcfd]">
                        <td className="px-5 py-4 font-semibold">{book.title}</td>
                        <td className="px-5 py-4 text-[#69758a]">{book.book_authors?.map(a => a.authors.name).join('، ')}</td>
                        <td className="px-5 py-4 text-[#69758a]">{book.category}</td>
                        <td className="px-5 py-4 text-[#69758a]">{book.printed_copies?.toLocaleString('ar-EG')}</td>
                        <td className="px-5 py-4 text-[#69758a]">{book.price_egp?.toLocaleString('ar-EG')} ج.م</td>
                        <td className="px-5 py-4 text-xs text-[#69758a]">{book.contract_date}</td>
                        <td className="px-5 py-4"><button className="rounded-lg bg-[#fff0ed] px-3 py-1.5 text-xs font-semibold text-[#d8573a]">عرض</button></td>
                      </tr>
                    ))}</tbody>
                  </table>
                  {!loading && filteredBooks.length === 0 && <p className="p-8 text-center text-sm text-[#9ba4b2]">لا توجد كتب مسجلة بعد.</p>}
                </div>
              </section>
            </>
          )}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-20 flex gap-1 overflow-x-auto border-t border-[#e8ebf0] bg-white px-2 py-2 lg:hidden">
        {navItems.map(({ label, href, icon: Icon }) => (
          <Link key={href} href={href} className={`flex min-w-[72px] flex-1 flex-col items-center gap-1 rounded-lg px-1 py-1 text-[10px] ${pathname === href ? 'text-[#d8573a]' : 'text-[#8d97a7]'}`}><Icon size={18} /><span className="truncate">{label}</span></Link>
        ))}
      </nav>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1a2540]/30 p-3 sm:p-6">
      <section className="my-3 w-full max-w-5xl rounded-2xl bg-white shadow-2xl sm:my-8">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-[#eef0f3] bg-white p-5">
          <div><h2 className="text-xl font-bold">إضافة كتاب جديد</h2><p className="mt-1 text-xs text-[#9ba4b2]">بيانات التعاقد والقسم الفني</p></div>
        ...
        <form onSubmit={onSubmit} className="grid gap-4 p-5 sm:grid-cols-2">
          {[['permit', 'اذن طباعة', 'text'], ['isbn', 'ترقيم دولي / ISBN', 'text'], ['title', 'اسم الكتاب', 'text'], ['copies', 'عدد النسخ المطبوعة', 'number'], ['freeCopies', 'عدد النسخ المجانية', 'number'], ['size', 'مقاس الكتاب', 'text'], ['profit', 'نسبة الارباح %', 'number'], ['phone', 'تليفون الكاتب', 'tel'], ['egp', 'السعر بالجنية', 'number'], ['aed', 'السعر بالدرهم', 'number'], ['sar', 'السعر بالريال', 'number'], ['date', 'تاريخ التعاقد او المعرض', 'date']].map(([id, label, type]) => (
            <label key={id}><span className="mb-2 block text-xs font-semibold text-[#69758a]">{label}</span><input required={id === 'title'} value={form[id]} onChange={e => setField(id, e.target.value)} type={type} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm outline-none focus:border-[#d8573a]" /></label>
          ))}
          <label>
            <span className="mb-2 block text-xs font-semibold text-[#69758a]">اسم الكاتب — يمكن إضافة أكثر من كاتب</span>
            <div className="flex gap-2"><input value={authorInput} onChange={e => setAuthorInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addAuthor())} placeholder="اكتب اسم الكاتب" className="min-w-0 flex-1 rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm" /><button type="button" onClick={addAuthor} className="rounded-xl bg-[#fff0ed] px-3 text-xs font-semibold text-[#d8573a]">إضافة</button></div>
            <div className="mt-2 flex flex-wrap gap-2">{form.authors.map((author: string) => <button type="button" key={author} onClick={() => setField('authors', form.authors.filter((item: string) => item !== author))} className="rounded-full bg-[#f7f8fa] px-3 py-1 text-xs">{author} ×</button>)}</div>
          </label>
          ...
          <label className="sm:col-span-2"><span className="mb-2 block text-xs font-semibold text-[#69758a]">النبذة</span><textarea value={form.summary} onChange={e => setField('summary', e.target.value)} rows={4} className="w-full rounded-xl border border-[#e8ebf0] px-3 py-3 text-sm" /></label>
          <div className="sm:col-span-2 flex items-center justify-between rounded-xl bg-[#f7f8fa] p-4">
            <div><p className="text-sm font-semibold">هل هذه طبعة جديدة لكتاب موجود؟</p><p className="mt-1 text-xs text-[#9ba4b2]">اربط السجل بكتاب سابق</p></div>
            <button type="button" onClick={() => setNewEdition(!newEdition)} className={`relative h-6 w-11 rounded-full transition ${newEdition ? 'bg-[#d8573a]' : 'bg-[#cfd5de]'}`}><span className={`absolute top-1 size-4 rounded-full bg-white transition ${newEdition ? 'right-1' : 'right-6'}`} /></button>
          </div>
          ...
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#d5dbe4] bg-[#fbfcfd] p-5 text-center"><Upload size={19} className="text-[#d8573a]" /><span className="mt-2 text-xs font-semibold text-[#69758a]">رفع ملف PDF للعقد</span><input type="file" accept="application/pdf" onChange={e => onContractChange(e.target.files?.[0] ?? null)} className="sr-only" /></label>
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-xl border border-dashed border-[#d5dbe4] bg-[#fbfcfd] p-5 text-center"><Upload size={19} className="text-[#d8573a]" /><span className="mt-2 text-xs font-semibold text-[#69758a]">رفع صورة الغلاف</span>{coverFile && <span className="mt-2 text-[11px] text-[#25824a]">{coverFile.name}</span>}<input type="file" accept="image/*" onChange={e => onCoverChange(e.target.files?.[0] ?? null)} className="sr-only" /></label>
          <div className="flex justify-end gap-3 border-t border-[#eef0f3] pt-5 sm:col-span-2">
            <button type="button" onClick={onClose} className="rounded-xl border border-[#e8ebf0] px-5 py-3 text-sm">إلغاء</button>
...
function SelectField({ label, value, onChange, options, valueMap }: any) {
  return <label><span className="mb-2 block text-xs font-semibold text-[#69758a]">{label}</span><select value={valueMap ? Object.keys(valueMap).find(k => valueMap[k] === value) ?? '' : value} onChange={e => onChange(valueMap ? valueMap[e.target.value] : e.target.value)} className="w-full rounded-xl border border-[#e8ebf0] bg-white px-3 py-3 text-sm"><option value="">اختر</option>{options.map((option: string) => <option key={option}>{option}</option>)}</select></label>
}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-[#1a2540]/30 p-3 sm:p-6">
      <section className="my-3 w-full max-w-4xl rounded-2xl bg-white shadow-2xl sm:my-8">
        <div className="flex items-center justify-between border-b border-[#eef0f3] p-5">
          ...
          <div className="flex items-center gap-3">
            <a href={`/books/${book.id}`} className="flex items-center gap-1.5 rounded-lg bg-[#fff0ed] px-3 py-2 text-xs font-semibold text-[#d8573a]"><ExternalLink size={14} />عرض الملف الكامل</a>
            <button onClick={onClose} aria-label="إغلاق"><X /></button>
          </div>
        </div>
        <div className="grid gap-4 p-5 sm:grid-cols-2">
          {rows.map(([label, value]) => <div key={label} className="rounded-xl bg-[#f7f8fa] p-4"><p className="text-xs text-[#9ba4b2]">{label}</p><p className="mt-1 text-sm font-semibold">{value || '—'}</p></div>)}
          {book.cover_image_url && <img src={book.cover_image_url} alt={`غلاف ${book.title}`} className="h-48 rounded-xl object-contain" />}
          {book.contract_pdf_url && <a href={book.contract_pdf_url} target="_blank" className="flex items-center justify-center rounded-xl border border-[#e8ebf0] p-4 text-sm font-semibold text-[#d8573a]">فتح ملف العقد PDF</a>}
        </div>
      </section>
    </div>
  )
